...
class DataTransformation:
    ...
    def get_data_transformer_object(self, input_df: pd.DataFrame):
        ...
        based on the input DataFrame's column types.
        """
        try:
            numerical_columns = input_df.select_dtypes(include=["int64", "float64"]).columns.tolist()
            categorical_columns = input_df.select_dtypes(include=["object", "bool"]).columns.tolist()
    ...
    def initiate_data_transformation(self, train_path: str, test_path: str):
        """
        This function reads train and test CSVs, applies transformation,
        and returns the transformed arrays along with the preprocessor path.
        """
        try:
